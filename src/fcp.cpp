...
std::pair<std::string, std::map<std::string, double>> Fcp::decode_msg(CANdata msg) {
	...
	int count = this->msgs.count(msg.sid);
	if (count == 0) {
		//return std::make_pair("", signals);
		uint16_t msg_id = GET_MSG_ID(msg.sid);
		uint16_t common_sid = GET_SID(0, msg_id);
		std::pair<std::string, std::map<std::string, double>> decoded = this->common_msgs[common_sid]->decode_msg(msg);
		decoded.first = this->devices[this->get_dev_name(decoded.second["dst"])].get_cmd_name(decoded.second["id"]);
		return decoded;
	}
	else {
		return this->msgs[msg.sid]->decode_msg(msg);
	}
}
